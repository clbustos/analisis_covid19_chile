...
Recuerde leer la [encuesta diaria realidad nacional de la SOCHIMI al 19 de Mayo](https://www.medicina-intensiva.cl/site/post_covid.php?id=58).
...
```{r}
plot.avance.pais(datos.casos[c("total","Metropolitana")],nvar = 'casos.nuevos', predicted = F,min.casos = 25, log.param =F, span.param = 0.5, con.etiqueta = FALSE) + geom_line(alpha=0.2)+guides(color=guide_legend("Región"))+ facet_wrap(~pais)+paleta+theme_bw()
```
...
```{r}
plot.avance.pais(datos.casos[c("total","Metropolitana")],nvar = 'casos.nuevos', predicted = F,min.casos = 25, log.param =T, span.param = 0.5, con.etiqueta = FALSE) + geom_line(alpha=0.2)+guides(color=guide_legend("Región"))+ facet_wrap(~pais)+paleta+theme_bw()
...
```{r}
plot.tasa.casos(datos.casos[c( "Valparaíso","Metropolitana","Ñuble","Biobío","Araucanía", "Maule","Tarapacá","Antofagasta","total")], span.param = NULL, casos.nuevos = T,ventana = 7)+ facet_wrap(~pais)+ guides(color=guide_legend("Región"))+ geom_abline(slope=0,intercept=1,col="darkgreen",alpha=0.5) +theme_bw()+paleta
```
...
Si observamos [la predicción](https://rpubs.com/clbustos/615041) realizada hace  `r f.actual-f.comp` día, podemos ver que T+AR(4) se acerca más al resultado observado que T+AR(1).
