---
title: "Análisis evolución Covid-19 para regiones de Chile (31/03/2020)"
...
Podemos constatar que la evolución de Santiago es la más fuerte del grupo, solo seguida por la Araucanía.

```{r}
plot.avance.pais(datos.pais,predicted = F,min.casos = 10,span.param = 0.7)+guides(color=guide_legend("Región"))+theme_bw()+paleta
```

Podemos ver en los gráficos independientes que regiones que se veían controladas dejan de estarlo, como Maule y Magallanes.  Preocupa que no se ven cambios relevantes de aceleración en Valparaíso.
```{r}
plot.avance.pais(datos.pais,predicted = F,min.casos = 10,span.param = 0.7)+guides(color=guide_legend("Región"))+facet_wrap(~pais)+paleta
```
...
y.max<-2200
x.max<-25
plot.avance.pais(datos.pais[c("Araucanía","Los.Ríos")],predicted = T,min.casos = 10, span.param = NULL)+facet_wrap(~pais)+scale_y_continuous(limits=c(NA,y.max),trans="log10")+guides(color=guide_legend("Región"))+scale_x_continuous(limits=c(0,x.max))+paleta
```
...
* Crecimiento moderado sin control: Presentan tasas entre 20% y 30%, pero sin curva de decrecimiento. Se encuentran aquí Los Lagos, Magallanes y Valparaíso.

```{r}
plot.avance.pais(datos.pais[c("Los.Lagos","Magallanes","Valparaíso")],predicted = T,min.casos = 10, span.param = NULL)+facet_wrap(~pais)+scale_y_continuous(limits=c(NA,y.max),trans="log10")+guides(color=guide_legend("Región"))+scale_x_continuous(limits=c(0,x.max))+paleta
```
...
Si observamos la proporción de casos nuevos por día, podemos ver claramente el surgimiento de aceleraciones en Coquimbo, Maule y Magallanes.
...
En general, podemos que las curvas AR son optimistas para todas las regiones, excepto para Maule.
...
Al igual que lo que vimos en días anteriores, podemos observar que si bien el modelo Tendencia +AR(1) tiende a entregar resultados optimistas a lo largo de la semana, termina prediciendo más casos que los modelos de curva exponencial y ARIMA(1,1,0). 
