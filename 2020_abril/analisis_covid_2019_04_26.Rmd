---
title: "Análisis evolución Covid-19 Chile casos y decesos (26/04/2020)"
...
Se puede observar que la tasa de aumento de la serie total y de la región Metropolitana se mantiene relativamente constante hace aproximadamente dos semanas. También podemos ver que Antofagasta y Tarapacá muestran ascensos en tasas que no han disminuido en el periodo. Atacama muestre un rápido ascenso en los últimos 5 días. Un patrón de ascenso rápido inicial, y tasas más pequeñas actuales se observan en Araucanía, Biobío, Coquimbo, Los lagos, Los Ríos, Valparaíso y Ñuble.
...
Con respecto a la serie de casos totales de  día vs el anterior, se mantiene constante desde hace días  una tasa de incremento diario de 10% para casi todo el Norte Grande: Arica y Parinacota - con una leve disminución el día de hoy-, Tarapacá y Antofagasta. Atacama ha presentado un incremento de casos importante en los últimos cinco días. 
...
Si observamos la tasa de nuevos casos por día, podemos ver en la serie total y la Metropolitana, pese a las variaciones diarias, sigue en una tendencia al ascenso.
...
Si observamos la evolución en el resto de las regiones, se pueden observar curvas de ascenso y control en Biobío, Los Lagos, Magallanes, Maule y Ñuble. Araucanía presentaba un patrón similar, pero desde hace 10 días se ha estancado en cerca de 30 casos días. En Valparaíso se observan dos procesos de aceleramiento y disminución, en tanto que Antofagasta, Arica y Parinacota y Tarapacá presentan patrones de ascenso. 
```{r}
...
Usando la tasa de casos nuevos diarios vs los del día anterior, con ventana de 7 días, se puede apreciar con mayor claridad las fuertes oscilaciones de Valparaíso, así como la tendencia la Región Metropolitana y de la serie total a incrementar los casos en una tasa constante.
...
En el caso del Maule, se puede apreciar que tras el fuerte incremento en casos, vuelve a mantener valores de tasa inferiores a 1 en la última semana.
...
Como ha sido la tónica durante la semana,  el modelo cuadrático es el más  pesimista y los modelos T+AR(1) y T+AR(4) tienden a converger. En una semana deberíamos tener entre 15300 y 17400 casos.
...
Durante la semana se ha visto que la predicción del método cuadrático y el de tendencia de casos nuevos comienza a converger, lo cual suele ocurrir cuando las tasas locales dejan de tener variaciones bruscas. Este modelo predice entre 16600 y 17200 casos en una semana.
...
ggplot(sec.basica, aes(x=dia,y=casos, color=tipo))+geom_point()+geom_line()+scale_y_continuous(trans="log10", limits=c(9000,17000),breaks = seq(8000,18000,1000))+xlim(dia.final-14,dia.final+8)
```
...
Se mantiene desde hace un par de días un intervalo de confianza superior elevado en Arica y Parinacota, Antofagasta y O'Higgins. Atacama presenta una predicción muy pesimista con AR(1), debido al fuerte incremento en casos de los últimos días.
...
Una primera serie es la de ventiladores disponibles. Durante los últimos dos días, los niveles del día son casi idénticos a los de la semana pasada (1082 vs 1083)
...
Si analizamos la serie total de pacientes en UCI para Covid-19, podemos ver un valor muy similar al del día de ayer, 415 vs 418.
...
Si observamos la serie total de decesos, observamos una tasa relativamente constante en las últimas dos semanas, atribuible a la región Metropolitana, ya que el resto de las series muestra menor cantidad de casos.
