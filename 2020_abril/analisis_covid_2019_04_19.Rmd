---
title: "Análisis evolución Covid-19 Chile casos y decesos (19/04/2020)"
...
Podemos observar que la tasa de casos nuevos ha permanecido constante desde aproximadamente una semana. Para la Región Metropolitana, Araucanía, Biobío y Valparaíso se observa una una leve aceleración. En el caso de Maule, observamos un fuerte incremento de casos en los últimos 5 días, con una pequeña desaceleración el día de ayer.
...
Con respecto a la cantidad de casos por día vs los del día previo, observamos aceleraciones en Arica y Parinacota, Tarapacá, Maule, Ñuble, Biobío, Araucanía y Magallanes. Se observan leves desaceleraciones en Valparaíso, Metropolitana y en la serie total.
...
En la serie de tasas de casos nuevos, que se genera con una ventana de 5 días, se aprecia con mayor claridad la aceleración de Ñuble y la Araucanía.
...
En el caso de Maule, se puede observar una pequeña desaceleración en los dos últimos días.
...
Si observamos la predicción a un  `r f.actual-f.comp` día plazo, podemos ver que el modelo cuadrático sobreestimo el valor observado, pero por poco, en tanto que los modelos T+AR(1) u T+AR(4) subestimaron en más de un 10%. En todos los casos, se cumplió el intervalo de confianza.
...
El modelo basado en AR(1) - no se pudo calcular por hoy cuadrático - muestra un total de 13028 en una semana. 
...
Hoy agregamos una serie nueva, la de ventiladores en uso y pacientes en UCI. Este dato es importante, porque es el que nos indica cuando se puede producir el colapso del sistema hospitalario.

Una primera serie es la de ventiladores disponibles. Desde el día 14 al día 19, donde hay disponible datos, se ha observado un aumento de 71 casos, de 986 a 1057 ventiladores ocupados. El total también ha aumentado prácticamente en la  misma proporción (77), por lo que la cantidad de ventiladores disponibles queda igual.

```{r}
rvd<-melt(datos.ministerio.bruto$vent.diario,id.vars = c("fecha","dia"))

ggplot(rvd,aes(x=as.Date(fecha),y=value,color=variable))+geom_line()+ylim(0,1700)+ylab("Nº de Ventiladores")+xlab("Fecha")
```

Si analizamos la serie total de pacientes en UCI para Covid-19, podemos ver que prácticamente se duplico desde inicios de mes hasta el 8 de Abril, para estabilizarse en cerca de 380 casos hasta la fecha.
```{r}
...
Al analizar la serie por regiones, podemos ver que en su mayoría los casos UCI se encuentran en la Región Metropolitana, siguiéndoles La Araucanía, pero con menos casos desde hace una semana.
```{r}
ggplot(datos.hosp.uci[datos.hosp.uci$region!="total",],aes(x=as.Date(fecha), y=casos, color=region))+geom_line()
```


## Decesos

Si observamos la serie total de decesos, podemos observar que la desaceleración presente hace tres semanas se ha detenido, y actualmente observamos una tasa mayor, desde hace 5 días. Con ciertas variaciones, la Región Metropolitana ha mantenido la misma tasa desde hace 10 días, así como la Araucanía hace praćticamente dos semanas. 
...
Vemos que la tasa de Ñuble ha sido relativamente constante en los últimos 10 días.
