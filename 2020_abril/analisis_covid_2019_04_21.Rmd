---
title: "Análisis evolución Covid-19 Chile casos y decesos (21/04/2020)"
...
En la serie total se puede apreciar un patrón de desaceleración lento, que parte hace 20 días. Este patrón parece ser producto de la estabilización de casos en diversas regiones, pero se ve contrarrestado por tasas estables o ascendentes en otras.
...
Con respecto a la cantidad de casos por día vs los del día previo, se puede ver un patrón ascendente en Arica y Parinacota,  Araucanía, Los Ríos, así como un patrón estable en Tarapacá, Antofagasta y Valparaíso.
...
En la serie de tasas de casos nuevos, con una ventana de 7 días, se aprecia un patrón descendente en la serie total, y en las regiones Metropolitana, Biobío y los Ríos. Sin embargo, se aprecia un patrón ascendente en Valparaíso, Ñuble y la Araucanía.
...
Si observamos la predicción a  `r f.actual-f.comp` días plazo, podemos ver que el modelo cuadrático sobreestimó el valor observado en tanto que los modelos T+AR(1) u T+AR(4) subestimaron, como ha sido la tónica durante la semana. Nuevamente, el intervalo se ha mantenido para todas las predicciones.
...
Podemos ver, al igual que en los días anteriores, que el modelo cuadrático es el más  pesimista, pero los modelos T+AR(1) y T+AR(4) tienden a converger. En una semana deberíamos tener entre 12500 y 14500 casos.
...
El modelo AR(1)  predice 13344, en tanto que el modelo cuadrático predice 14436.
...
Eliminaremos la predicción de Atacama y Aysén, que presentan tasas estabilizadas. Se pudo reincorporar Maule, que por su bruscos cambios no generaba una predicción estable para el modelo cuadrático.
...
Una primera serie es la de ventiladores disponibles. Se aprecia una tendencia al aumento de los ventiladores ocupados, de 986 a 1093 en 8 días.
...
Si analizamos la serie total de pacientes en UCI para Covid-19, podemos ver que prácticamente se duplico desde inicios de mes hasta el 8 de Abril, para estabilizarse en cerca de 380 casos hasta el 20 de Abril. Hoy 21 tenemos 392 casos en UCI.
```{r}
...
Al analizar la serie por regiones, podemos ver que en su mayoría los casos UCI se encuentran en la Región Metropolitana, siguiéndoles La Araucanía y Biobío, ambas con series descendentes desde aproximadamente 5 días. Es relevante notar el aumento fuerte en los casos UCI de los últimos 4 días.
```{r}
...
Si observamos la serie total de decesos, observamos una leve desaceleración de la tasa global. La tasa de la región metropolitana muestra una leve desaceleración en los últimos dos días. 
