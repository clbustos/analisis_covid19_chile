---
title: "Análisis evolución Covid-19 Chile casos y decesos (12/04/2020)"
...
**Comentario técnico**: Intenté modelar una biexponencial sobre los casos nuevos, pero no convergía el modelo. Una Gompertz si funciona, pero es demasiado optimista, ya que asume que estamos viendo el máximo punto de la crisis. Por tanto, me mantengo con los modelos que llevo hasta ahora.


## Análisis de series de casos


La serie de casos se hace hace partir de los 25 casos, excepto para Atacama y Aysén, donde se inicia del quinto caso, para facilitar la comparación desde un punto de un punto de partida común.Se mantiene la tendencia de la serie total de mantenerse en un crecimiento exponencial sostenido sin mucha desaceleraión, supongo que impulsado por la tasa constante de la metropolitana, que lleva casi 2 semanas. En otras regiones de la zona centro y Sur , como Araucanía, Ñuble y Biobío, ya observamos la clásica curva de desaceleración. 
...
Si observamos la proporción de casos nuevos por región, observamos aceleraciones en Tarapacá y Antofagasta. La tendencia de los últimos días es a volver a desacelerar, lo cual indica que la tendencia al alza de los últimos dos o tres días puede que haya sido un influjo de nuevos casos específicos.
...
En el  gráfico FSDG, se observa la inestabilidad de tasa de Arica y Parinacota, así como de Atacama.
...
Como hicimos ayer, verifiquemos el desempeño de los algoritmos durante el presente periodo. Hace `r f.actual-f.comp` días, se predijo para hoy 6655 y 6739 casos con AR(1) y AR(4), respectivamente, ambas predicciones bajo el valor observado. Esto nos indica que la semana pasada la predicción era más optimista de lo que debido, aunque los intervalos de confianza incluyen el dato observado.
...
El modelo de sumas de T+AR(1) en 7 días predice 9977, 800 casos más que los que predicen los otros modelos.
...
Si observamos la serie de decesos, tanto la serie como total como para la Región Metropolitana sigue un patrón exponencial, con tasas del 21% y 16.7% diario, respectivamente. En el caso de la Araucanía, se han disminuido los decesos atribuidos a Covid-19 durante la última semana. 
