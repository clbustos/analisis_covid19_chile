---
title: "Análisis evolución Covid-19 Chile casos y decesos (25/04/2020)"
...
Con respecto a la serie de casos totales de  día vs el anterior, se mantiene constante desde hace días  una tasa de incremento diario de 10% para casi todo el Norte Grande: Arica y Parinacota, Tarapacá y Antofagasta. Atacama ha presentado un incremento de casos importante en los últimos tres días. 
...
Si observamos la evolució en el resto de las regiones, se pueden observar curvas de ascenso y control en Biobío, Los Lagos, Magallanes, Maule y Ñuble. En Valparaíso se observan dos procesos de aceleramiento y disminución, en tanto que Antofagasta, Arica y Parinacota, Tarapacá y Araucanía van en ascenso. 
```{r}
...
Usando la tasa de casos nuevos diarios vs los del día anterior, con ventana de 7 días, se puede apreciar con mayor claridad las oscilaciones de Valparaíso, así como la tendencia la Región Metropolitana y de la serie total a incrementar los casos en una tasa constante.
...
Como ha sido la tónica durante la semana,  el modelo cuadrático es el más  pesimista y los modelos T+AR(1) y T+AR(4) tienden a converger. En una semana deberíamos tener entre 14800 y 17000 casos.
...
Durante la semana se ha visto que la predicción del método cuadrático y el de tendencia de casos nuevos comienza a converger, lo cual suele ocurrir cuando las tasas locales dejan de tener variaciones bruscas. Este modelo predice entre 16000 y 16600 casos en una semana.
...
ggplot(sec.basica, aes(x=dia,y=casos, color=tipo))+geom_point()+geom_line()+scale_y_continuous(trans="log10", limits=c(9000,17000),breaks = seq(8000,17000,1000))+xlim(dia.final-14,dia.final+8)
```
...
Una primera serie es la de ventiladores disponibles. Los niveles actuales son casi idénticos a los de la semana pasada (1058 vs 1057)
...
Si analizamos la serie total de pacientes en UCI para Covid-19, podemos ver un nuevo máximo en la serie con 418 casos
...
Si observamos la serie de decesos por día, podemos ver que ha seguido una progresión ascendente desde el segundo día, estabilizándose en la última semana.
