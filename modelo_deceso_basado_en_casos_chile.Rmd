...
La serie de Chile tiene datos para  `r n` días. Si calculamos las correlaciones entre el número de casos y decesos por día, podemos ver que la máxima correlación se observa con un retraso de 5-7 días, siendo alta alta también entre los días 11-16 y en el día 21.
```{r}
c1<-cor(chile)[1,-1]
plot(0:22,c1 )
```
...
El R²=0.82  es más bajo que el del modelo completo.
```{r}
...
Probemos a validar el modelo previo. Podemos ver que el optimismo es mucho menor, con un R² positivo, lo que nos indica 

```{r}

ols.2<-ols(decesos~casos.1+casos.6+casos.8+casos.12+casos.13,chile,x=T,y=T)
validate(ols.2)
```
