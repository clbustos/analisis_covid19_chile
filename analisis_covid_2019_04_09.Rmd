---
title: "Análisis evolución Covid-19 Chile casos y decesos (09/04/2020)"
...
library(pander)
datos.casos<-lapply(datos.casos,function(x) {x<-x[x$fecha<=43929,]})
datos.decesos<-lapply(datos.decesos,function(x) {x<-x[x$fecha<=43929,]})
...
Si observamos la proporción de casos nuevos por región, observamos que Arica y Parinacota se encuentraba en una fase de expansión muy brusca, que ya se estaría controlando con un 20% el día de hoy. En todas las otras regiones se encuentra el patrón esperado de disminución diaria de casos por día al menos en los últimos 3 días, con diversos niveles de avance, excepto en Atacama, nuevamente. 
...
En el  gráfico FSDG, donde se observa la diferencia diaria en tasas, lo ideal que los valores se encuentran bajo 1, ya que indica que la tasa disminuye a un ritmo acelerado. Podemos ver que en varias series se encuentran pequeñas aceleraciones, como en Coquimbo, Metropolitana, Maule, Los Lagos.
...
Hemos eliminado la serie exponencial, ya que se aleja mucho del patrón observado. Agregamos hace un par de días una serie de tendencia + AR(4), ya que al observar los residuos del modelo T+AR(1) veamos autocorrelaciones a 3 y 4 días plazo.

Hoy se puede observar que las series T+AR(1) y T+AR(4) convergen en la predicción para 7 días más.
...
El modelo de sumas de T+AR(1) en 7 días predice 8092 vs un poco más de 7820 casos de los otros dos modelos.
...
Si observamos la serie de decesos, vemos que la serie nacional sigue un patrón exponencial. En el caso de la Araucanía se observa una disminución de las muertes en los últimos 4 días, en tanto que el proceso parece acelerarse en la Región Metropolitana.
